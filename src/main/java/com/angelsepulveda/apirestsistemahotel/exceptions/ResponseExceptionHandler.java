...
public class ResponseExceptionHandler extends ResponseEntityExceptionHandler{
    ...
    public final ResponseEntity<ResponseException> manejarTodasExcepciones(Exception ex, WebRequest request) {
        ResponseException er = new ResponseException(LocalDateTime.now(), ex.getMessage(), request.getDescription(false));
        return new ResponseEntity<ResponseException>(er, HttpStatus.INTERNAL_SERVER_ERROR);
    }

    @ExceptionHandler(ModelNotFoundException.class)
    public final ResponseEntity<ResponseException> manejarModeloException(ModelNotFoundException ex, WebRequest request) {
        ResponseException er = new ResponseException(LocalDateTime.now(), ex.getMessage(), request.getDescription(false));
